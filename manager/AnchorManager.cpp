...
void AnchorManager::showCurrent() {
    ...
    QQuickWindow* mainWindow = m_controller->guiManager()->getMainWindow();
    if (!mainWindow) return;
    double dp = m_controller->guiManager()->getGuiScaling();

    qreal pageCenterX = block->getGuiX() - 15*dp + (1280*dp / 2.0);
    qreal pageCenterY = block->getGuiY() - 15*dp + (720*dp / 2.0);
    qreal x = pageCenterX - mainWindow->width() / 2.0;
    qreal y = pageCenterY - mainWindow->height() / 2.0;

    m_controller->guiManager()->setWorkspacePosition(qRound(-x), qRound(-y));
}
...
void AnchorManager::startPresentationMode() {
    ...
    for (BlockInterface* anchor: m_anchors.values()) {
        anchor->hideGui();
    }
    showCurrent();
}
...
void AnchorManager::removeAnchor(BlockInterface* block) {
    ...
    if (m_currentIndex >= m_anchors.size()) {
        setCurrentIndex(qMax(m_anchors.size() - 1, 0));
    }
    emit anchorsChanged();
...
int AnchorManager::getAnchorIndex(BlockInterface* block) const {
    if (!m_anchorOrder.contains(block->getUid())) {
        return -1;
    }
