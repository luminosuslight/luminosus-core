...
TouchAreaEvent& TouchAreaEvent::operator=(const TouchAreaEvent& other) {
    m_isValid = other.m_isValid;
    m_x = other.m_x;
    m_y = other.m_y;
    m_originX = other.m_originX;
...
void TouchArea::touchEvent(QTouchEvent* event) {
    if (event->type() == QEvent::TouchCancel) {
        qInfo() << "Touch was canceled by system.";
        if (m_secondTouch.isValid()) {
            onTouchUpInternal(m_secondTouch, true);
        }
        if (m_firstTouch.isValid()) {
    ...
    for (const QTouchEvent::TouchPoint& touchPoint: event->touchPoints()) {
        ...
        case Qt::TouchPointPressed:
            if (event->touchPoints().size() == 1 && m_firstTouch.isValid() && m_firstTouch.isTouch()) {
                // m_firstTouch is still valid, but there is only one touch and that one is new
                // -> m_firstTouch was canceled without notice
                onTouchUpInternal(touch, true);
            }
...
void TouchArea::onTouchDownInternal(TouchAreaEvent& touch) {
    ...
    if (!touch.isAccepted()) {
        // we won't get a touch up event in this case, tidy up now:
        if (m_secondTouch.isValid()) {
...
void TouchArea::mouseDoubleClickEvent(QMouseEvent* event) {
    TouchAreaEvent touch(event, this);
    m_mouseLast = touch;
    emit doubleClick(&m_mouseLast);  // FIXME: this emits a second doubleClick
}
