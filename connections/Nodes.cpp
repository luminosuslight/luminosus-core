...
Command* NodeBase::getCommand() {
    if (m_command.isNull()) {
        qCritical() << "NodeBase::getCommand(): Command is empty";
    }
    return m_command;
}
...
void NodeBase::receiveCommand(Command* command) {
    if (m_isOutput) {
        qCritical() << "Method receiveCommand() is only available for input nodes.";
        return;
    }
    m_command = command;
...
void NodeBase::sendCommand(Command* command) {
    if (!m_isOutput) {
        qCritical() << "Method sendCommand() is only available for output nodes.";
        return;
    }
    m_command = command;
    ...
    for (NodeBase* inputNode: m_connectedNodes) {
        if (!inputNode) continue;
        inputNode->receiveCommand(command);
    }
}
